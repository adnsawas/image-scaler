...
describe('Testing getImage API', () => {
    it('fetches original image without scaling', async () => {
        ...
        let isFileSaved: boolean = fs.existsSync(path.join(__dirname, '../../assets/images/pikachu.jpg'));
    ...
    it('fetches original image with scaling height only', async () => {
        ...
        let isFileSaved: boolean = fs.existsSync(path.join(__dirname, '../../assets/images/pikachu_h_200.jpg'));
        expect(isFileSaved).toEqual(true);
    });
    ...
    it('fetches original image with scaling width only', async () => {
        ...
        let isFileSaved: boolean = fs.existsSync(path.join(__dirname, '../../assets/images/pikachu_w_200.jpg'));
        expect(isFileSaved).toEqual(true);
    });

    it('fetches original image with scaling height and width', async () => {
        await request.get('/api/getImage?fileName=pikachu&height=200&width=200').expect(200);
        let isFileSaved: boolean = fs.existsSync(path.join(__dirname, '../../assets/images/pikachu_h_200_w_200.jpg'));
        expect(isFileSaved).toEqual(true);
    });
});